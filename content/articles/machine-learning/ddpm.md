---
title: DDPM 的 Tensor 运算与代码解析
date: 2026-03-25
summary: 从正向加噪、时间步嵌入和简化 UNet 出发，梳理 DDPM 的公式与 PyTorch 实现。
minutes: 28
---

以下是对ddpm原理和代码的简单分析。

代码仓库：[github代码仓库](https://github.com/NJU-AIA)中.
...
其中 $\sqrt{\alpha_t\cdots\alpha_2}\sqrt\beta_1\epsilon_1+\sqrt{\alpha_t\cdots\alpha_3}\sqrt\beta_2\epsilon_2+\cdots+\sqrt\alpha_t\sqrt\beta_{t-1}\epsilon_{t-1}+\sqrt\beta_t\epsilon_t$
正好是多个独立的正态噪声之和，其均值为0，方差则分别为$\alpha_t\cdots\alpha_2\beta_1$、$\alpha_t\cdots\alpha_3\beta_2$、$\cdots$、$\alpha_t\beta_{t-1}$、$\beta_t$。
...
graph TB
    ...
    %% 瓶颈层
    Down2 --> Bottle[bottleneck
    Conv2d 3x3
    通道:128→128]

    %% 解码器路径（上采样）
    Bottle --> Up1[up1
    ConvTranspose2d 4x4/2
    通道:128→64
    尺寸:7→14]
    Up1 --> Cat1[拼接 enc2 输出]
    Enc2 -- 跳跃连接--> Cat1
    ...
    %% 样式设置
    classDef encoder fill:#fbb,stroke:#f66,color:#000;
    classDef decoder fill:#bfb,stroke:#3c3,color:#000;
    classDef bottle fill:#aaf,stroke:#33f,color:#000;
    classDef skip fill:#ddd,stroke:#999,color:#000;
    classDef time fill:#ffd,stroke:#fc0,color:#000;

    class A,Bottle time;
    class Enc1,Enc2,Down1,Down2 encoder;
    class Dec1,Dec2,Up1,Up2,Out decoder;
...
还有文件加载和展示图片部分代码未展示，详见[github代码仓库](https://github.com/NJU-AIA)中.
